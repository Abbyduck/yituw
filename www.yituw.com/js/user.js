 ...
 * 修改会员信息
 */
function userEdit_user()
{
  var frm = document.forms['formEdit'];
  var extend_field102 = frm.elements['extend_field102'].value;
  var msg = '';
  var reg = null;
...
function editPassword()
{
  ...
  if (old_password.length == 0)
  {
    ...
    dialog("请输入您的原密码！",1);return false;
  }

  if (new_password.length == 0)
  {
    msg += new_password_empty + '\n';
    dialog("请输入您的新密码！",1);return false;
  }

  if (confirm_password.length == 0)
  {
    msg += confirm_password_empty + '\n';
    dialog("请输入您的确认密码！",1);return false;
  }
  ...
  if (new_password.length > 0 && confirm_password.length > 0)
  {
    if (new_password != confirm_password)
    {
      ...
      dialog("您现两次输入的密码不一致！",1);return false;
    }
  }
  if (msg.length > 0)
  {
...
function is_registered( username )
{
  ...
  if ( username == '' )
    ...
    Ajax.call( 'http://www.yituw.com/js/user.php?act=is_registered', 'username=' + username, registed_callback , 'GET', 'TEXT', true, true );
}
...
function checkEmail(email)
{
  ...
  Ajax.call( 'http://www.yituw.com/js/user.php?act=check_email', 'email=' + email, check_email_callback , 'GET', 'TEXT', true, true );
}
...
{
  Ajax.call('http://www.yituw.com/js/user.php?act=return_to_cart', 'order_id=' + orderId, returnToCartResponse, 'POST', 'JSON');
}
...
function validate_vcode(code){
  if(captcha != 1){
    ...
    filter.code    = code;
    Ajax.call('http://www.yituw.com/js/user.php?is_ajax=1&act=ajax_validate_vcode', filter, validate_vcode_result, 'GET', 'JSON')
  }
}
...
function send_sms(type)
{
  if(wait == 120){
    ...
    Ajax.call('http://www.yituw.com/js/user.php?act=ajax_validate_sms', filter, send_result, 'POST', 'JSON');
  }
}
function send_sms(){
  if(wait == 120){
...
function rep_validate_code(code){
  var filter = new Object;
  filter.code    = code;
  Ajax.call('http://www.yituw.com/js/user.php?is_ajax=1&act=ajax_validate_vcode', filter, rep_validate_code_result, 'GET', 'JSON')
}
...
function rep_send_sms(){
  if(wait == 120){
    ...
    var filter = new Object;
    filter.mobile = document.getElementById("username").value;
    Ajax.call('http://www.yituw.com/js/user.php?act=ajax_validate_sms', filter, send_result_rep, 'POST', 'JSON');
  }
}
