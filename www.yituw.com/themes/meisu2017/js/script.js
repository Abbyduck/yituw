...
//	表格收起
	...
	$('#srchimg_wrap_fa input[type=radio]').click(function(){
        // n = $(this).index();

        // alert(n)
        // console.log(n)
        // $('.srchimg_wrap_fa li').removeClass('selected').eq(n).addClass('selected');
    })
  ...
  //alert(1)
		...
		$(this).find('.del').click(function(e){	
			layer.open({
			  ...
			  yes: function(index){
		  		...
		  		loading(id);
				$.get("http://www.yituw.com/themes/meisu2017/js/flow.php?step=drop_goods",{id:id},
				    function(data){
				    loading(2);	
				      if(data.error=='0'){
				      		f.hide();	
	...
	$('.all_del').click(function(e){		
		...
		layer.open({
			  ...
			  yes: function(index){
			   	loading(1);
			$.post("http://www.yituw.com/themes/meisu2017/js/flow.php?step=clear",
			{
				ids:ids
			},
			    function(data){
...
//	$( "#datepicker" ).datepicker();
	...
	$('#getcode').on('click',function(event){
		...
		loading(1);
		$.post("http://www.yituw.com/themes/meisu2017/js/user.php?act=ajax_validate_sms",
			{
	...
	$('#getcodepasw').on('click',function(event){
		...
		loading(1);
		$.post("http://www.yituw.com/themes/meisu2017/js/user.php?act=ajax_misspasw_sms",
			{
				mobile:$("#paswtel").val()
			},
		    function(data){
	...
	//鼠标点击变红
	$(".imgList,.container").on("click","dd.sp1.htm"/*tpa=http://www.yituw.com/themes/meisu2017/js/dd.sp1*/,function(e){
	//$('.ui_cover .sp1').click(function(e){
		...
		var goods        = new Object();
  		...
  		loading(1);
		$.post("http://www.yituw.com/themes/meisu2017/js/flow.php?step=gy_add_to_cart",
		    {
		      goods:goods
		    },
		    function(data){
			...
			$(e.target).next().show();
				dialog('加入购物车成功',2);
			  	...
			  	//window.location.href="http://www.yituw.com/themes/meisu2017/js/user.php?act=login";
			  	$("#login").click();
			  }else{
			  	dialog(data.message,1);
			  }
		...
		$.post("http://www.yituw.com/themes/meisu2017/js/flow.php?step=gy_add_to_cart",
		    ...
		    function(data){
		      ...
		      if(data.error=='0'){
			   ...
			   window.location.href="http://www.yituw.com/themes/meisu2017/js/flow.php?step=cart&ok=1";
	...
	$('#tc_buy').click(function(e){
        $('#tc_buy').attr("disabled","disabled");
		  ...
		  goods.number   = number;
  		loading(1);
		$.post("http://www.yituw.com/themes/meisu2017/js/flow.php?step=add_to_cart",
		    {
		    ...
		    function(data){
		      	loading(2);
		      if(data.error=='0'){
				window.location.href="http://www.yituw.com/themes/meisu2017/js/flow.php?step=done";
			  }else if(data.error=='10'){
			  	dialog(data.message,1);
			  }else{
			  	dialog(data.message,1);
	...
	$(".imgList,.container").on("click","dd.sp2.htm"/*tpa=http://www.yituw.com/themes/meisu2017/js/dd.sp2*/,function(e){
...
////  		loading(1);
  		...
  		layer.open({
			...
			closeBtn: 1,
			shade:0.8, 
			area: ['850px', '440px'], //瀹介珮
			content: $('#srchimg_wrap_fa'),
			yes: function(index){
				layer.close(index);
			},
		...
		// $.get("http://www.yituw.com/themes/meisu2017/js/user.php?act=collect",{goods_sn:goods.goods_sn,goods_title:goods.goods_title},
		//     function(data){
		//     loading(2);	
	...
	//$('.ui_cover .sp2_1').click(function(e){	
		...
		var goods        = new Object();
  		...
  		loading(1);
		$.get("http://www.yituw.com/themes/meisu2017/js/user.php?act=delete_collection_bysn",{goods_sn:goods.goods_sn,goods_title:goods.goods_title,class_id:goods.class_id},
		    function(data){
		    	loading(2);
	...
	//搜索结果:查看相似图片 by gy
	$(".imgList,.container").on("click","dd.sp3.htm"/*tpa=http://www.yituw.com/themes/meisu2017/js/dd.sp3*/,function(e){
	//$('.ui_cover .sp3').click(function(){	
		var goods        = new Object();
  		goods.goods_sn = $(this).parent().attr("sn");
  		goods.goods_title = $(this).parent().attr("title");
  		window.location.href="http://www.yituw.com/themes/meisu2017/js/similar.php?media_id="+goods.goods_sn;
	})
	//收藏到-无自建 1》
	$('#transfer_n').click(function(){
		...
		layer.open({
		  ...
		  yes: function(index){
				...
				loading(1);
				$.post("http://www.yituw.com/themes/meisu2017/js/user_api_collection.php?act=move",
					{
	...
	$('#del_goods').click(function(){
		...
		layer.open({
		  ...
		  yes: function(index){
				loading(1);
				$.post("http://www.yituw.com/themes/meisu2017/js/user_api_collection.php?act=del",
					{
						sn:sn
					},
				    function(data){
	...
	//收藏到-有自建
	$(".imgList,.container").on("click","dd.sp4.htm"/*tpa=http://www.yituw.com/themes/meisu2017/js/dd.sp4*/,function(e){
	...
	$('#del').click(function(){
		layer.open({
		  ...
		  yes: function(index){
		    		var class_id= $('#cont .on').attr("cid");
				loading(1);
				$.post("http://www.yituw.com/themes/meisu2017/js/user_api_collection.php?act=delclass",
					{
						cid:class_id
					},
				    function(data){
				    loading(2);	
				      if(data.error=='0'){
				      		layer.close(index);
				      		//dialog(data.message,2);
				      		windows_url('http://www.yituw.com/themes/meisu2017/js/user.php?act=collection_list');
					  }else{
					  	dialog(data.message,1);
					  }
				    },"json");
		...
		layer.open({
		  ...
		  yes: function(index){
		    var class_id= $('#cont .on').attr("cid");
				...
				$.post("http://www.yituw.com/themes/meisu2017/js/user_api_collection.php?act=cmm",
					{
...
//	关注和取消关注
	...
	$('#btn_cartcheck').click(function(event) {
		...
		$.post("http://www.yituw.com/themes/meisu2017/js/flow.php?step=gy_move_to_cart",
			{
				ids:ids
			},
		    function(data){
...
//	关注和取消关注
	...
	$("#ajax_done").click(function(e){
		...
		loading(1);
		$.post("http://www.yituw.com/themes/meisu2017/js/user.php?act=ajax_buy_botton",
			{
				pid:pid,payid:payid
			},
		    function(data){
		    loading(2);
		      ...
		      if(data.error=='0' || data.error == 0){
				  ...
				  btn: ['支付成功', '支付遇到问题'],
				  yes: function(index){
				    	windows_url("http://www.yituw.com/weixinredirect.php?order_id="+pid+"");
				  },
				  btn2:function(){
				  	window.location.href = "../../../about.php-action=questions.htm"/*tpa=http://www.yituw.com/about.php?action=questions*/;
				  }
				});
		      		if(payid=='5'){
		      			$("#llpaysubmit").submit();
		      			...
		      			var wx = "<input id=\"weixinbotton\" onclick=\"window.open('http://www.yituw.com/user.php?act=pay_weixin&order_id="+pid+"')\" value=\"立即支付\" type=\"button\">";
	...
	$("#accountset_btn").click(function(event) {
		...
		loading(1);
		$.post("http://www.yituw.com/themes/meisu2017/js/design.php?act=ajax_accountset",
			{
	...
	$('#getcodebank').on('click',function(event){
		...
		loading(1);
		$.post("http://www.yituw.com/themes/meisu2017/js/design.php?act=ajax_bank_sms",
			{
	...
	$(".authorized_default").click(function(event) {
		loading(1);
		$.post("http://www.yituw.com/themes/meisu2017/js/user.php?act=authorized_default",
			{
				id:$(this).attr("id")
			},
		    function(data){
	...
	$(".authorized_del").click(function(event) {
		...
		layer.open({
		  ...
		  yes: function(index){
				loading(1);
				$.post("http://www.yituw.com/themes/meisu2017/js/user.php?act=authorized_del",
					{
						id:aid
					},
				    function(data){
	...
	$(".subaccount_edit").click(function(event) {
		...
		$.get("http://www.yituw.com/themes/meisu2017/js/user.php?act=subaccount_edit_info",{id:sid},
				    function(data){
				      if(data.error=='0'){
						  ...
						  yes: function(index){
						    loading(1);
							$.post("http://www.yituw.com/themes/meisu2017/js/user.php?act=subaccount_edit",
								{
	...
	$('#designset_save').click(function(event) {
		...
		loading(1);
		$.post("http://www.yituw.com/themes/meisu2017/js/design.php?act=designset",
			{
	...
	$('.jubao').on('click', function(event) {
		...
		loading(1);	
		$.post("http://www.yituw.com/themes/meisu2017/js/jubao.php",
			{
